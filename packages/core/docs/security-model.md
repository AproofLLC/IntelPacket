...
This document describes how `@intelpacket/core` fails closed on bad inputs, limits resource use, and keeps hashing deterministic. It is not a formal audit report.
...
Constants (see `src/constants.ts` and package exports) cap:
...
`@intelpacket/core` does not provide PII tokenization or field-level privacy transforms. Callers must normalize or redact sensitive data before packetization if required by policy.
...
- Prefer `pnpm run typecheck`, `pnpm test`, and `pnpm run build` before release.
